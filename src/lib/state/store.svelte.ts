// The application "brain": a Svelte 5 runes store that owns the reactive UI
...
import { MetronomeEngine, type BeatInfo, type MetronomeHooks } from '$lib/audio/metronome';
import { resumeAudio } from '$lib/audio/context';
import { bpmFromTapIntervalMs, clampBpm, formatDuration } from '$lib/audio/timing';
import {
	gapMute,
	rampTempoByBars,
	rampTempoByTime,
...
import { MicTempoDetector } from '$lib/tempo/micTempo';
import {
	addSession,
	aggregate,
	clearSessions,
...
import type { GoalType, PracticeGoal, PracticeSession } from '$lib/practice/types';
...
class Store {
	...
	/** Call once on the client (e.g. from onMount). Idempotent. */
	init(): void {
